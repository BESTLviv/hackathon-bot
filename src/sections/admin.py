...
class AdminSection:
    ...
    def send_admin_menu(self, call=None):
        ...
        start_btn = InlineKeyboardButton(
            start_btn_text, callback_data=start_btn_callback
        )
        # Stop Hack Button
        ...
        team_list_btn = InlineKeyboardButton(
            team_list_btn_text, callback_data=team_list_btn_callback
        )
        # Add Team Button
        add_team_btn_text = "Добавити команду"
        add_team_btn_callback = self.form_admin_callback(action="AddTeam")
        add_team_btn = InlineKeyboardButton(
            add_team_btn_text, callback_data=add_team_btn_callback
        )
        ...
        send_msg_to_all_btn_callback = self.form_admin_callback(
            action="SendMessageToAll"
        )
        send_msg_to_all_btn = InlineKeyboardButton(
            send_msg_to_all_btn_text, callback_data=send_msg_to_all_btn_callback
        )
        ...
        send_msg_to_game_btn_callback = self.form_admin_callback(
            action="SendMessageToParticipants"
        )
        send_msg_to_game_btn = InlineKeyboardButton(
            send_msg_to_game_btn_text, callback_data=send_msg_to_game_btn_callback
        )
        ...
        photo_change_btn = InlineKeyboardButton(
            change_btn_text, callback_data=change_btn_callback
        )
        ...
        schedule_photo_change_btn = InlineKeyboardButton(
            change_btn_text, callback_data=change_btn_callback
        )
        ...
        description_change_btn = InlineKeyboardButton(
            change_btn_text, callback_data=change_btn_callback
        )
        admin_markup.add(
            photo_change_btn, schedule_photo_change_btn, description_change_btn
        )
        ...
        time_left_photo_change_btn = InlineKeyboardButton(
            change_btn_text, callback_data=change_btn_callback
        )
        ...
        mentor_section_change_btn = InlineKeyboardButton(
            change_btn_text, callback_data=change_btn_callback
        )
        ...
        add_partner_btn = InlineKeyboardButton(
            add_partner_btn_text, callback_data=add_partner_btn_callback
        )
        admin_markup.add(add_partner_btn)

        if call is None:
            self.bot.send_message(
                chat_id=self.data.ADMIN_CHAT_ID,
                text="КУКУ адмін",
    ...
    def send_team_list(self, call):
        ...
        for team in team_list:
            ...
            btn_callback = self.form_admin_callback(
                action="TeamDetail", team_name=team["name"]
            )
    ...
    def send_team_detail(self, call, team_name):
        ...
        btn_callback = self.form_admin_callback(
            action="SendMessageToTeam", team_name=team_name
        )
        btn = InlineKeyboardButton(btn_text, callback_data=btn_callback)
        markup.add(btn)
        ...
        change_task_btn_callback = self.form_admin_callback(
            "ChangeTeamTask", team_name=team_name
        )
        change_task_btn = InlineKeyboardButton(
            change_task_btn_text, callback_data=change_task_btn_callback
        )
        ...
        btn_callback = self.form_admin_callback(
            action="SendTeamTasks", team_name=team_name
        )
        btn = InlineKeyboardButton(btn_text, callback_data=btn_callback)
        markup.add(btn)
    ...
    def send_admin_message_to_team(self, team_name):
        text_to_admin = (
            f"Надішли мені повідомлення і я надішлю його команді <b>{team_name}</b>!"
        )
        self.bot.send_message(
            chat_id=self.data.ADMIN_CHAT_ID, text=text_to_admin, parse_mode="HTML"
        )

        self.bot.register_next_step_handler_by_chat_id(
            self.data.ADMIN_CHAT_ID,
    ...
    def send_admin_message_to_all(self):
        text_to_admin = (
            "Надішли мені повідомлення і я надішлю його всім користувачам бота!"
        )
        self.bot.send_message(
            chat_id=self.data.ADMIN_CHAT_ID, text=text_to_admin, parse_mode="HTML"
        )
        ...
        self.bot.register_next_step_handler_by_chat_id(
            self.data.ADMIN_CHAT_ID, self.process_send_message, destination="ALL"
        )
    ...
    def process_send_message(self, message, **kwargs):
        ...
        for user in users:
            ...
            try:
                if photo is None:
                    self.bot.send_message(
                        chat_id=user_chat_id, text=text, parse_mode="HTML"
                    )
                else:
                    self.bot.send_photo(
        ...
        self.bot.send_message(
            chat_id=self.data.ADMIN_CHAT_ID, text="Успішно відправлено!"
        )
